...
import {
    ...
    shuffleArray as systemmyHelperjs_shuffleArray,
...
function Enemy (tile, map, type) {
    ...
    this.direction = systemmyHelperjs_getRandomInt(0,3);
...
Enemy.prototype.checkForCrossRoad = function ()
  ...
  if (!this.isSolidForEnemy(x,y-1)) roads.push(0);
      ...
      if (roads.indexOf(this.direction) >= 0 && roads.length > 1)
        {
          var wannaChangeDirection = systemmyHelperjs_getRandomInt(0,9);
          if (wannaChangeDirection < this.bonus.roadChanging) return this.direction;
        }
          }
  roads = systemmyHelperjs_shuffleArray(roads);
  return roads[0];
};
...
var exported_Enemy = Enemy;
export { exported_Enemy as Enemy };
