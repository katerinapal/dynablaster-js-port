...
import {
    getRandomInt as myHelper_getRandomInt,
    pickRandomProperty as myHelper_pickRandomProperty,
...
StageNumberState.prototype.reset = function () {
    var bonuses = 4;
    var mapSize = {
        x : 6 + this.game.gamePlayer.map + myHelper_getRandomInt(5, 10),
        y : 6 + this.game.gamePlayer.map + myHelper_getRandomInt(0, 5),
    };
    ...
    while(enemiesList.length <= enemiesCount)
        {
            var chosenEnemy = myHelper_pickRandomProperty(Enemy.types);
            if (Enemy.types[chosenEnemy].minMap <= this.game.gamePlayer.map &&
                ...
                myHelper_getRandomInt(0,9) < Enemy.types[chosenEnemy].prob)
...
var mod_StageNumberState = StageNumberState;
export { mod_StageNumberState as StageNumberState };
