...
Game.prototype.handleInput = function (type) {
    if (keyboard.pressed(systemKeyjs_S))
        {
            sounds.soundsChange();
            this.gamePlayer.updateSounds();
        }

    if (keyboard.pressed(systemKeyjs_H))
        {
        this.gameStateManager.switchTo("hi_score").reset();
        }
    //
    if (keyboard.pressed(systemKeyjs_P) && this.gameStateManager.getLastMasterStateId() === "game_state_playing")
        {
        this.gameStateManager.switchTo('game_pause');
        }

    if (keyboard.pressed(systemKeyjs_Q) && this.gameStateManager.getLastMasterStateId() === "game_state_playing")
        {
        this.gameStateManager.switchTo('game_over').reset();
        }

    if (keyboard.pressed(systemKeyjs_escape))
        {
