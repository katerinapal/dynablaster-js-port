...
import {
    space as systemKeyjs_space,
    left as systemKeyjs_left,
    up as systemKeyjs_up,
    right as systemKeyjs_right,
    down as systemKeyjs_down,
    B as systemKeyjs_B,
} from "..\\system\\Key.js";
...
{
    if (keyboard.down(systemKeyjs_space))
    {
    ...
    if (keyboard.down(systemKeyjs_B) && this.bonus.detonator)
    {
        this.parent.activateBombs();
    }

    if (keyboard.down(systemKeyjs_left) && !keyboard.down(systemKeyjs_right))
        {
    ...
    if (keyboard.down(systemKeyjs_right) && !keyboard.down(systemKeyjs_left))
        {
            if (this.tilePosition.v &&
                    ((!this.isSolidForPlayer(this.tilePosition.xTile+1, this.tilePosition.yTile) ||
    ...
    if (keyboard.down(systemKeyjs_up) && !keyboard.down(systemKeyjs_down))
        {
    ...
    if (keyboard.down(systemKeyjs_down) && !keyboard.down(systemKeyjs_up))
        {
            if (this.tilePosition.h &&
                    ((!this.isSolidForPlayer(this.tilePosition.xTile, this.tilePosition.yTile+1) ||
...
var exported_Player = Player;
export { exported_Player as Player };
